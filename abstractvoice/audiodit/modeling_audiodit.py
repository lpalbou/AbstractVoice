...
class AudioDiTRotaryEmbedding(nn.Module):
    ...
    def _build(self, seq_len: int, device: torch.device, dtype: torch.dtype):
        """Build cos/sin tables entirely on CPU (matching original behaviour), then move to target."""
...
class AudioDiTBlock(nn.Module):
    ...
    def __init__(self, config: AudioDiTConfig):
        ...
        if config.dit_cross_attn:
            ...
            self.cross_attn_norm = (
                nn.LayerNorm(dim, elementwise_affine=True, eps=eps) if config.dit_cross_attn_norm else nn.Identity()
            )
    ...
    def forward(
        ...
        if self.adaln_type == "local" and adaln_global_out is None:
            ...
            assert adaln_global_out is not None
...
class AudioDiTTransformer(nn.Module):
    ...
    def __init__(self, config: AudioDiTConfig):
        ...
        if config.dit_text_conv:
            self.text_conv_layer = nn.Sequential(
                *[
                    AudioDiTConvNeXtV2Block(dim, dim * 2, bias=config.dit_bias, eps=config.dit_eps)
                    for _ in range(4)
                ]
            )
    ...
    def forward(
        ...
        if self.use_latent_condition:
            assert latent_cond is not None
...
class _VaeEncoderBlock(nn.Module):
    def __init__(
        ...
        layers.append(
            _wn_conv1d(in_ch, out_ch, kernel_size=2 * stride, stride=stride, padding=math.ceil(stride / 2))
        )
...
class AudioDiTVaeEncoder(nn.Module):
    def __init__(self, config: AudioDiTVaeConfig):
        ...
        ch = config.channels
        layers: list[nn.Module] = [_wn_conv1d(config.in_channels, c_mults[0] * ch, kernel_size=7, padding=3)]
...
class AudioDiTVaeDecoder(nn.Module):
    def __init__(self, config: AudioDiTVaeConfig):
        ...
        layers: list[nn.Module] = [_wn_conv1d(config.latent_dim, c_mults[-1] * ch, kernel_size=7, padding=3)]
        ...
        layers.append(
            _wn_conv1d(c_mults[0] * ch, config.in_channels, kernel_size=7, padding=3, bias=False)
        )
    ...
    def forward(
        ...
        if text_embedding is not None:
            ...
            if attention_mask is not None:
                ...
                text_condition_len = torch.full((text_condition.shape[0],), text_condition.shape[1], device=device)
        else:
