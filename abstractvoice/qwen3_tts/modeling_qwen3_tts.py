# Vendored from qwen-tts 0.1.1 (https://github.com/QwenLM/Qwen3-TTS, Apache-2.0),
# upstream file: qwen_tts/core/models/modeling_qwen3_tts.py
# Shipped inside AbstractVoice to enable offline-first inference without
# trust_remote_code and without the upstream SDK's pinned dependency set
# (transformers==4.57.3, accelerate, gradio, librosa, sox, onnxruntime).
# Local modifications, listed exhaustively; everything else is upstream-verbatim:
#   1. Qwen3TTSTokenizer import redirected to abstractvoice.qwen3_tts.codec (upstream's inference wrapper pulls librosa/base64/URL loading).
#   2. librosa.filters.mel replaced by ._mel.librosa_mel_fn (thin adapter over transformers.audio_utils.mel_filter_bank; parity 3.7e-9).
#   3. ROPE_INIT_FUNCTIONS lookups routed through _hf_compat.rope_init_fn ('default' key dropped in transformers 5.x); config.pad_token_id reads via _hf_compat.pad_token_id_of (attribute removed in 5.x).
#   4. _tied_weights_keys list form -> dict form (transformers 5.x expects a mapping; the list form breaks save_pretrained).
#   5. create_causal_mask keyword renamed input_embeds -> inputs_embeds at every call site
#      (the alias is deprecated-since-5.6 and maps silently today; the rename removes the debt).
#   6. Decode-step position handling: the prefill/decode split is derived from the KV cache
#      and the step length from inputs_embeds -- transformers 5.x generate() no longer
...
from .._hf_compat import pad_token_id_of, rope_init_fn
from transformers.utils.hub import cached_file

from .codec import Qwen3TTSCodec as Qwen3TTSTokenizer
...
class Qwen3TTSTalkerForConditionalGeneration(Qwen3TTSTalkerTextPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if attention_mask is not None:
            # Local modification (transformers >= 5.x): generate() no longer
            # maintains `cache_position` for in-repo models (only trust_remote_code
            # models get a BC shim), so upstream's prefill/decode split on
            # `cache_position[0] == 0` never saw a decode step and rebuilt
            # positions for the WHOLE history each token. Derive the phase from
            # the KV cache itself, and the step length from inputs_embeds (this
            # forward builds it above; upstream's input_ids.shape relied on 4.57
            # passing only the last step's ids).
            past_seen_tokens = past_key_values.get_seq_length() if past_key_values is not None else 0
